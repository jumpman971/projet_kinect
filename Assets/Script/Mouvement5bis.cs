...
public class Mouvement5bis : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (state == 0) {
            ...
            if ((goingRightY == 1 && goingLeftY == 0) || (goingRightY == 1 && goingLeftY == -1) && Vector2.Distance(mh.currPosRight, startPosRight) > mh.minMove) {
                state = 2;
                action = true;
            } else if (((goingRightY == 0 && goingLeftY == 1) || (goingRightY == -1 && goingLeftY == 1)) && Vector2.Distance(mh.currPosLeft, startPosLeft) > mh.minMove) {
                state = 3;
                action = true;
            ...
            if (goingRightY == -1 && goingLeftY == 1 && Vector2.Distance(mh.currPosLeft, startPosLeft) > mh.minMove) {
                state = 4;
                action = true;
            } else if (mh.GetMouvementTimeout())
                state = 0;
            ...
            if (goingRightY == 1 && goingLeftY == -1 && Vector2.Distance(mh.currPosRight, startPosRight) > mh.minMove) {
                state = 5;
                action = true;
            } else if (mh.GetMouvementTimeout())
                state = 0;
